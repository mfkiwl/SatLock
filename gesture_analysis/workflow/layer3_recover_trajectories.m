...
for i = 1:n_case
    ...
    gallery_cfg = build_gallery_cfg_local(cfg.data_cfg, inject_case.true_label);
...
trajectory.template_order = injected.template_order;
end
...
function cfg_out = build_gallery_cfg_local(cfg_in, true_label)
cfg_out = cfg_in;
if ~isfield(cfg_out, 'track') || ~isstruct(cfg_out.track)
    cfg_out.track = struct();
end
