...
flowchart TD
    ...
    B --> C["Attack Layer: 攻击模拟<br/>replay / sdr_spoof / ghost_injection"]
...
- `gesture_analysis/results/paper_figures_data_driven_output/paper_figures_data_driven_*/`
  这是论文图目录，保存 PNG 和 `paper_figure_manifest.csv`。
...
当前这三项指标使用与 ROC 一致的 operating point 计算，默认采用 `EER threshold`。
...
### 9.9 `rmse_confusion_matrix.png`

`RMSE_k` 矩阵图。看不同真实类别与各模板之间的平均 RMSE。

### 9.10 `dtw_confusion_matrix.png`

`DTW_k` 矩阵图。看不同真实类别与各模板之间的平均 DTW。

### 9.11 `attack_defense_boxplot.png`
...
### 9.12 `attack_defense_rates.png`

攻击防御率柱状图。

当前口径：

- 基于无攻击认证 ROC 得到的 `EER threshold` 固定工作点；
- 第一组为 `Legitimate`，展示合法用户的 `TAR / FRR`
...
### 9.15 `height_sensitivity_dual_axis.png`
